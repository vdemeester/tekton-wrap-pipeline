...
func Pipeline(ctx context.Context, rrclientSet rrclientset.Interface, pipelineClientSet clientset.Interface, namespace, pipelineRef string) (*v1beta1.Pipeline, error) {
	...
	if strings.Contains(pipelineRef, "resolver:") {
		...
		name, err := resource.GenerateDeterministicName(string(ref.Resolver), "wrap", ref.Params)
		if err != nil {
			return nil, err
		}
		rr, err := rrclientSet.ResolutionV1alpha1().ResolutionRequests(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			...
			rr, err = rrclientSet.ResolutionV1alpha1().ResolutionRequests(namespace).Create(ctx, request, metav1.CreateOptions{})
			if err != nil {
				return nil, err
			}
		}
		...
		err = rrclientSet.ResolutionV1alpha1().ResolutionRequests(namespace).Delete(ctx, name, metav1.DeleteOptions{})

		return pipeline, nil

	}
	pipeline, err := pipelineClientSet.TektonV1beta1().Pipelines(namespace).Get(ctx, pipelineRef, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	return pipeline, nil
